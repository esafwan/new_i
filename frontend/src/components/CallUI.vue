...
async function updateNote(_note, insert_mode = false) {
  ...
  if (insert_mode && _note.name) {
    await call('new_i.integrations.twilio.api.add_note_to_call_log', {
...
async function is_twilio_enabled() {
  return await call('new_i.integrations.twilio.api.is_enabled')
}
...
async function startupClient() {
  ...
  try {
    const data = await call('new_i.integrations.twilio.api.generate_access_token')
